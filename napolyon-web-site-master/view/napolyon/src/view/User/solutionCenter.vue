          ...
          <div class="col-lg-12">
            <div class="questions-text">
              <transition name="fade">
                <p v-if="activated">
                  <template>
                    <h2 class="questions-subtitle">{{ $t("solutionCenter.help_text") }}</h2>
                  </template>
                </p>
              </transition>
            </div>
                ...
                <p v-if="activated">
                  <template>
                    <input v-model="search"
                           class="form-control form-control-lg" placeholder="Konuyu yazınız.." type="text">
                  </template>
                </p>
              </transition>
            </div>
        ...
        <div class="row justify-content-center">
          <div class="col-lg-12 col-md-12">
              ...
              >
                <transition name="bounce">
                  <p v-show="item.show && !search">
                    <a
                        :class="{ active: item.solutionCenterId === activeItemId }"
            ...
            <transition-group name="list-complete" tag="p">
              <div v-for="(item, index) in childItems" :key="index"
                   class="tab-content questions-tab-content list-complete-item d-block">
                <div v-if="item.show" class="show active tab-pane fade questions-tab-pane" role="tabpanel">
                  <div id="withdrawal-accordion" class="questions-accordion">
                    <div class="card questions-card">
                      <div class="card-header questions-card-header">
                        <h5 class="mb-0">
                            <span class="btn btn-link collapsed" @click="showDetails(item)">
                              <h4>{{ item.title }}</h4>
                            </span>
                        </h5>
                      </div>
                        ...
                        <div class="card-body questions-card-body">
                          ...
                          <div class="question-wizard">
                            ...
                            <div class="buttons d-flex justify-content-center">
                              <vue-reaction-emoji v-for="(subItem, subIndex) in item.question.options"
              ...
              <p v-if="detailItem && showDetail">
                <template>
                  <div class="tab-content questions-tab-content">
                    <div class="show active tab-pane fade questions-tab-pane" role="tabpanel">
                          ...
                          <div class="questions-show">
                            <div class="card-body questions-card-body">
                              ...
                              <div class="question-wizard">
                                ...
                                <div class="buttons d-flex justify-content-center useful-actions">
                                  <template v-for="(subItem, subIndex) in detailItem.question.options">
                                   <span v-if="subItem.isUseful === 1" class="btn btn-link collapsed h5"
                                         @click="reactionClicked(subItem)">
                                     Evet
                                   </span>
                                    <span v-if="subItem.isUseful === 0" class="btn btn-link collapsed h5"
                                          @click="reactionClicked(subItem)">
                                    Hayır
                                   </span>
                                  </template>
      ...
      get() {
        ...
        if (item && item.hasOwnProperty('childs') && item.childs) {
          item.childs.map(child => {
            if (typeof child.question === 'string' || child.question instanceof String)
  ...
  methods: {
    ...
    submit() {
      ...
      if (+self.topic !== 16 && !self.user) {
        self.contactUsError = "Talebinizi iletmek için Giriş yapmanız gerekmektedir.";
    ...
    sendSolutionCenterHit() {
      ...
      this.$store
          .dispatch(_MODULE_NAME_SOLUTION + "/" + HIT_ITEM, {
            url: HIT_URL,
            contents: formData,
            isAuthenticated: self.isAuthenticated,
          })
