<template>
  <div id="contentUs">
    ...
    <section v-if="!isMobile"
             class="bg-half bg-light d-table w-100" style="background: url('media/images/images/contact-detail.jpg') center center;">
              ...
              <div class="page-next">
                <nav aria-label="breadcrumb" class="d-inline-block">
                  <ul class="breadcrumb bg-white rounded shadow mb-0">
                    <li class="breadcrumb-item"><a @click="$router.push({name:'user.index'})">{{ $t('index.home') }}</a>
                    </li>
                    <li aria-current="page" class="breadcrumb-item active">Anket Sonucu</li>
                  </ul>
                </nav>
              </div>
            </div>
      ...
      <div class="container">
        <div class="row text-center mb-20" v-if="surveyResult">
          <div v-if="surveyResult == surveyFilled" class="col-lg-12">
            <h3 v-if="isMobile" class="primary-color">Sevgili {{ username }},</h3>
            <h1 v-else class="primary-color">Sevgili {{ username }},</h1>
            <p>Anketimize gösterdiğiniz ilgi için teşekkürler fakat bu anketimizi daha önce doldurdunuz. </p>
            <p>Yeni bir ankette görüşmek üzere ...</p>
            <p class="mt-5">Saygılarımızla, <span class="mt-1 d-block">Napolyon.com</span></p>
          </div>
          ...
          <div v-if="surveyResult == linkError ||
          surveyResult == messageError ||
          surveyResult == linkError ||
          surveyResult == messageNotFoundError ||
          surveyResult == offerError" class="col-lg-12">
            <h1 class="primary-color">Sevgili {{ username }},</h1>
            <p>Anketimize gösterdiğiniz ilgi için teşekkürler.</p>
            <p>Teknik bir sorundan kaynaklı anket puanınız hesabınıza maalesef yüklenememiştir.</p>
            <p>Konu ilgili birimlere iletilmiş olup, en kısa zamanda çözüme kavuşturulacaktır.</p>
            <p class="mt-5">Saygılarımızla, <span class="mt-1 d-block">Napolyon.com</span></p>
          </div>

          <div v-if="surveyResult == overQuota" class="col-lg-12">
            <h1 class="primary-color">Sevgili {{ username }},</h1>
            <p>Bulunduğunuz grupta hedeflenen kişi sayısına ulaşılmıştır. Göstermiş olduğunuz ilgi için teşekkür ederiz.</p>
            <p class="mt-5">Saygılarımızla, <span class="mt-1 d-block">Napolyon.com</span></p>
          </div>
          ...
          <div v-if="surveyResult == timeUp" class="col-lg-12">
            ...
            <p>Size ulaşan anketleri anında doldurun, NAP puan kazanma şansınızı ve Napolyon Mobil Uygulamasına özel
              Liderlik sıralamasındaki KUPA sayınızı arttırın. </p>
            <p>Göstermiş olduğunuz ilgi için teşekkür ederiz. Yeni bir ankette görüşmek üzere.</p>
            <p class="mt-5">Saygılarımızla, <span class="mt-1 d-block">Napolyon.com</span></p>
            <div class="alert alert-success" v-if="cup" >
              Tebrikler <strong>{{ cup }}<img :src="mediaBase + '/images/cup.png'" height="18" width="18"/> </strong>
              kazandınız.
            </div>
          </div>
          ...
          <div v-if="surveyResult == complete" class="row p-3">
            <div class="col-lg-6 col-md-6 col-xs-12">
              <img :src="mediaBase + '/images/smile.png'" class="img-responsive text-center animated rubberBand mt-3">
              <br/>
              <p style="text-align:center; margin-top: 10px; font-family: museo500; font-weight:bold; color: #008679; font-size: x-large;">
                TEBRİKLER KAZANDINIZ!</p>
            </div>
            <div class="col-lg-6 col-md-6 col-xs-12 text-left">
              <h3 v-if="isMobile" class="primary-color">Sevgili {{ username }},</h3>
              <h1 v-else class="primary-color">Sevgili {{ username }},</h1>
              <p>Bu anketi doldurarak <strong>{{ point }}</strong> NAP Puan <span v-if="cup"> ve aşağıda belirtilen kadar kupa </span> kazandınız.
              </p>
              <p v-if="cup">Kupalarınız Napolyon Mobil Uygulamasındaki Liderlik sıralamasını belirler.</p>
              <p>Göstermiş olduğunuz ilgi için teşekkür ederiz. Yeni bir ankette görüşmek üzere.</p>
              <p>Saygılarımızla,</p>
              <p>Napolyon Ekibi</p>
            </div>
          ...
          <div v-if="surveyResult == screenOut" class="row p-3">
            <div class="col-lg-6 col-md-6 col-xs-12">
              <img :src="mediaBase + '/images/sad.png'" class="img-responsive text-center animated rubberBand mt-3">
              <br/>
              <p style="text-align:center; margin-top: 10px; font-family: museo500; font-weight:bold; color: #008679; font-size: x-large;">
                ÜZGÜNÜZ BU ANKET SİZE GÖRE DEĞİL!</p>
            </div>
            <div class="col-lg-6 col-md-6 col-xs-12 text-left">
              <h3 v-if="isMobile" class="primary-color">Sevgili {{ username }},</h3>
              <h1 v-else class="primary-color">Sevgili {{ username }},</h1>
              <p>Maalesef görüşleri alınmak istenen hedef grubun içerisinde değilsiniz.</p>
              <p><span v-if="point"><strong>{{ point }}</strong> Nap Puan </span>  <span v-if="point && cup"> ve </span><span v-if="cup"> KUPA ödülü </span> hesabınıza yüklendi.</p>
              <p>Kupalarınız Napolyon Mobil Uygulamasındaki Liderlik sıralamasını belirler.</p>
              <p>Göstermiş olduğunuz ilgi için tekrar teşekkür ederiz. Yeni bir ankette görüşmek üzere.</p>
              <p>Saygılarımızla,</p>
              <p>Napolyon Ekibi</p>
            </div>
            <div v-if="cup" class="col-12 mt-2">
...
import module, {GET_USER, MODULE_NAME, USER} from "@/core/services/store/auth.module";
    ...
    getResult() {
      ...
      this.$store.dispatch(GET_ITEMS, {
        ...
        if (response.status) {
          ...
          self.screenMember = self.result.hasOwnProperty('screenMember') ? response.data.screenMember : null;
...
*/
    ...
    if (this.$route.name == 'index.survey_result_2' && this.$route.query.hasOwnProperty('mid') && this.$route.query.hasOwnProperty('ca')) {
      this.mid = this.$route.query.mid;
      this.ca = this.$route.query.ca;
      this.getResultType();
    }

    if (this.$route.name == 'index.survey_result' && this.$route.query.hasOwnProperty('mid')) {
      this.mid = this.$route.query.mid;
      this.getResult();
    }
