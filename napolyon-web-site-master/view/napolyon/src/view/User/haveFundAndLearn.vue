  ...
  <div class="col-lg-8 col-12">
    <div class="rounded shadow border-bottom p-4">
      <div class="row">
        ...
        <div class="col-lg-4 col-md-6 col-12 mt-2 pt-2 text-center" v-for="(game, index) in games" :key="index">
          <div class="card pricing-rates business-rate shadow bg-light border-0 rounded">
            <div class="card-body">
              <h6 @click="$router.push({name:'user.games.'+game.name})" :class="{active: $route.name === 'user.games.'+game.name}" class="title fw-bold text-uppercase text-primary mb-4">{{ $t('haveFundAndLearn.'+game.name) }}</h6>
              <div class="d-flex mb-4 game-box" @click="$router.push({name:'user.games.'+game.name})" :class="{active: $route.name === 'user.games.'+game.name}">
                <img :src="mediaBase + '/images/icons/'+game.icon">
              </div>
              <a @click="$router.push({name:'user.games.'+game.name})" :class="{active: $route.name === 'user.games.'+game.name}" class="btn btn-primary mt-4">{{$t('haveFundAndLearn.play')}}</a>
            </div>
          </div>
        </div>
      </div>
