      ...
      >
        <div v-if="status" id="game-area" class="row">
          ...
          >
            <div
              v-if="question"
              class="col-lg-12 col-md-12 col-12 question text-center"
            >
              <img
          ...
          >
            <a
              ...
              v-on:click.prevent="sendAnswer(item)"
              :class="isAnswerSending ? 'disabled' : ''"
            >
              <div class="question">
                <img
                  v-if="item.option_image"
                  :src="getQuestionAnswerImage(item)"
                  width="60"
...
export default {
  ...
  methods: {
    getQuestionAnswerImage(item) {
      ...
      if (
        item.option_image &&
        item.option_image === "icon_correct_answer.png"
      ) {
        imageName = "yes.png";
      } else if (
        item.option_image &&
        item.option_image === "icon_wrong_answer.png"
      ) {
        imageName = "no.png";
      }
      ...
      axios
        ...
        .then((response) => {
          ...
          axios
            .post(
              process.env.VUE_APP_MOBIL_URL +
                "duels_web/send_start_duel_request.php?" +
            ...
            .then((response) => {
              ...
              if (response.data.status == 0) {
                if (
                  response.data.hasOwnProperty("subject_id") &&
                  response.data.subject_id != 0
                ) {
                  ...
                  Swal.fire({
                    ...
                    if (response.isConfirmed) {
                      ...
                      axios
                        .post(
                          process.env.VUE_APP_MOBIL_URL +
                            "duels_web/send_join_duel_request.php?" +
                            querystring.stringify(params),
                          {},
                          this.config
                        )
                ...
                let html =
                  ...
                  '<img src="' +
                  response.data.content[0][2] +
                  '" style="border: 3px solid black;border-radius: 50%;" width="100"><span class="pt-2 d-block">' +
                  response.data.content[0][1] +
                  "</span>" +
                  "</div>" +
                  '<div class="col-md-6 overflow-hidden">' +
                  '<img src="' +
                  response.data.content[1][2] +
                  '" style="border: 3px solid black;border-radius: 50%;" width="100"><span class="pt-2 d-block">' +
                  response.data.content[1][1] +
                  "</span>" +
                  "</div>" +
    ...
    getQuestion: function (response = null) {
      ...
      axios
        .get(
          process.env.VUE_APP_MOBIL_URL +
            "duels_web/get_duel_question_items.php?" +
            querystring.stringify(params),
          {},
          this.config
        )
          ...
          if (response.data.status == "0") {
            ...
            let params = {
              ...
              processControlBlockId:
                self.proccessControlId != null
                  ? self.proccessControlId
                  : response.data.part3,
            };
            axios
              .get(
                process.env.VUE_APP_MOBIL_URL +
                  "duels_web/get_duel_result_items.php?" +
                  querystring.stringify(params),
                {},
                this.config
              )
              .then((response) => {
                ...
                let html =
                  ...
                  '<img src="' +
                  result.user_photo +
                  '" style="border: 2px solid black;border-radius: 50%;" width="60">' +
                  '<div class="d-inline-block pl-3">' +
                  "<span>" +
                  result.user_name +
                  "</span>" +
                  '<h5 style="color:#0a9c8c;">' +
                  result.user_detail +
                  "</h5>" +
                  "</div>" +
                  "</div>" +
              ...
              .catch((error) => {
                ...
                Swal.fire({
                  ...
                  self.$router.push({ name: "index.duel" });
                });
              });
          }
        })
    ...
    sendAnswer: function (answer) {
      ...
      let params = {
        ...
        duelId: self.duel_id,
        level: self.level,
        answerTime: answer.hasOwnProperty("answer_time")
          ? answer.answer_time
          : 30 - this.start,
      };
      ...
      axios
        .post(
          process.env.VUE_APP_MOBIL_URL +
            "duels_web/send_duel_question_answer_request.php?" +
            querystring.stringify(params),
          {},
          this.config
        )
