...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      if (!this.validateEmail(self.email)) {
        if (!Number.isInteger(Number(self.email)) || String(self.email).length !== 10) {
              ...
              if (response.status) {
                ...
                try {
                  ...
                  if (item && item.hasOwnProperty("content")) {
                    self.error = item.content || "İşlem yapılırken hata oluştu. Lütfen daha sonra tekrar deneyiniz";
                  } else if (item) {
  ...
  mounted() {
    ...
    if (this.$route.name == 'index.auto_login') {
      ...
      store.dispatch(_MODULE_NAME + "/" + GET_USER, data)
          .then((response) => {
            self.$store.commit(_MODULE_NAME + "/" + SET_USER, response);
            self.$router.push({path: "uyelik"});
          });
